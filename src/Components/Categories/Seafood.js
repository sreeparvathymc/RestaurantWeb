...
const Seafood = () => {
  ...
  useEffect(() => {
    const fetchItems = async () => {
      try {
        const response = await fetch("/data.json"); // Ensure the correct path to your data.json
        if (!response.ok) {
          throw new Error("Network response was not ok");
        }
        const data = await response.json();
        const seafoodItems = data.foods.filter(item => item.category === "seafood");
        setFoods(seafoodItems);
      } catch (error) {
                ...
                <Link to={`/view/${item.id}`}>
                  <div className="flex gap-5">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-left font-medium mt-3">Price: {item.price}/-</p>
                    </div>
                  </div>
                </Link>
